...
export async function updateImage({ image, userId, path }: UpdateImageParams) {
	try {
		...
		const updateImage = await Image.findByIdAndUpdate(imageToUpdate.id, image, { new: true });
...
export async function getAllImages({
	...
	try {
		...
		let exporession = 'folder=visioncraft';

		if (searchQuery) {
			exporession += ` AND ${searchQuery}`;
		}

		const { resources } = await cloudinary.search.expression(exporession).execute();
